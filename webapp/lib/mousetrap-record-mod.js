// Modified by Pete to support 'in progress events'
...
(function(Mousetrap) {
    ...
    function _handleKey(character, modifiers, e) {
        ...
        // console.log( _currentRecordedKeys);
        // prevent defaults
        if (e.preventDefault) {
            e.preventDefault();
        } else {
    ...
    function _normalizeSequence(sequence) {
        for (var i = 0; i < sequence.length; ++i) {
            sequence[i] = _formatSequence(sequence[i]);
        }
    }

    function _formatSequence(sequence) {
    ...
    function _getCurrentState() {
        ...
        var current = [];
        if( _currentRecordedKeys && _currentRecordedKeys.length ) {
            current.push(_formatSequence(_currentRecordedKeys));
        }
        for (var i = _recordedSequence.length; i-- > 0; ) {
            var key = _formatSequence(_recordedSequence[i]);
            if( key === "" ) {
                throw _recordedSequence[i];
            }
            current.unshift( key );
        }
        return current;
     ...
     * @returns void
     */
    Mousetrap.record = function(callback, inprogressCallback) {
        if( inprogressCallback ) {
            Mousetrap.handleKey = function () {
                _handleKey.apply(this,arguments);
                inprogressCallback.apply(this, [_getCurrentState()]);
            }
        } else {
